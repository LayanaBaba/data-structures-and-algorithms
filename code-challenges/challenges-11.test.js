...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(arr=>arr.filter(item=>((typeof item)==='number')&&(item%5===0)).map(item=>Math.pow(2,item))
  )
//   let element;
//   for (let i=0; i<input.length; i++){
// element = arr.filter(item=>((typeof item)==='number')&&(item%5===0)).map(item=>Math.pow(2,item))
//   }
//   return element
};
