...
------------------------------------------------------------------------------------------------ */
const toLastNames = people => people.map(arr=>`${arr.firstName}`+' '+`${arr.lastName}`)
...
const addValues = (arr) => {
  // Solution code here...
  let sum = arr.reduce((acc,val,inx)=>{
    acc = acc + val;
    return acc; 
  },0);
  return sum
...
const countNumberOfElements = (arr) => arr.reduce((item)=>item+1)
  // Solution code here...
